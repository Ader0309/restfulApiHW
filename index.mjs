...
app.get("/api/users/status", checkToken, async (req, res) => {
    ...
    user = await getUserByAccount(account)
        .then((result) => result)
        .catch((err) => {
            error = err;
            console.log(err);
...
app.put("/api/users/:id/", checkToken, upload.none(), async (req, res) => {
    // res.send(`更新特定ID的使用者 ${req.params.id}`);
    let user, error;
    user = await userModify(req)
        .then((result) => result)
        .catch((err) => {
            error = err;
            console.log(err);
...
app.delete("/api/users/:id/", checkToken, async (req, res) => {
    // res.send(`刪除特定ID的使用者 ${req.params.id}`);
    let user, error;
    user = await userDelete(req)
        .then((result) => result)
        .catch((err) => {
            error = err;
            console.log(err);
